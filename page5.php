...
if (isset($_GET['vegan'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM  diet WHERE diet.name =?");
...
if (isset($_GET['meth'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM  method WHERE method .name =?");
    $stmt->bind_param("s", $nam);
    $stmt->execute();
...
<?php include('./header.php')?>
            ...
            <div id="content" class="super_collection super_collection_wrapper2 " >
              ...
              <div class="gid_warap">
                <section id="product1" class="collection_carousel collection_carousel_product section-p1">
                  ...
                  <?php  include('./get_vegn.php')?>
                    <?php if (isset($featurd_products)) {
        while($row = $featurd_products->fetch_assoc()){   ?>
        ...
        echo "No products found."; // Handle case where no products are fetched
    } ?>
